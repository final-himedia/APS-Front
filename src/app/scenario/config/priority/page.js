...
export default function PriorityView() {
  ...
  const fetchData = (token, id) => {
    ...
    fetch(`15.164.98.31:8080/api/scenarios/config/priority/${id}`, {
  ...
  const handleFileChange = (e) => {
    ...
    fetch("15.164.98.31:8080/api/scenarios/config/priority-upload", {
      method: "POST",
      body: formData,
      headers: {
  ...
  const handleDownload = () => {
    ...
    fetch(
      `15.164.98.31:8080/api/scenarios/config/priority-download?scenarioId=${scenarioId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
