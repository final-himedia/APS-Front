...
export default function BomView() {
  ...
  const fetchData = (token, scenarioId) => {
    ...
    fetch(`http://15.164.98.31:8080/api/scenarios/bop/bom/${scenarioId}`, {
      headers: {
  ...
  const handleDownload = () => {
    if (!token || !scenarioId) return;
    fetch(
      `http://15.164.98.31:8080/api/scenarios/bop/bom-download?scenarioId=${scenarioId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleFileChange = (e) => {
    ...
    fetch("http://15.164.98.31:8080/api/scenarios/bop/bom-upload", {
      method: "POST",
      body: formData,
      headers: {
        Authorization: `Bearer ${token}`,
