...
export default function DataGridSection() {
  ...
  const setSelectedScenarioId = useScenarioStore((state) => state.setSelectedScenarioId);
  ...
  // ✅ 데이터 가져오기
  const fetchBomData = (id) => {
    const url = id
      ? `http://localhost:8080/api/scenarios/bop/bom/${id}`
      : `http://localhost:8080/api/scenarios/bop/bom`;
    ...
    fetch(url)
      ...
      .catch((err) => console.error("BOM 데이터 가져오기 실패:", err));
  };

  // ✅ 시나리오 ID 변경되면 데이터 fetch
  useEffect(() => {
    if (scenarioId) fetchBomData(scenarioId);
  }, [scenarioId]);
  ...
  const handleDownloadExcel = () => {
    window.location.href = "http://localhost:8080/api/scenarios/bop/bom-download";
  };
      ...
      <Box sx={{ mt: 2 }}>
        ...
        <Dialog open={open} onClose={handleCloseDialog} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            ...
            >
              <Typography color="text.secondary">등록할 파일을 선택하세요.</Typography>
              <input
                id="file-input"
                type="file"
                accept=".xlsx,.xls"
